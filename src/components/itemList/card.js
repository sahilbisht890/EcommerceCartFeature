    ...
    return (
        <div className="d-flex flex-column gap-2 w-25 justify-content-end" key={id}>
            ...
            <div className='d-flex justify-content-between align-items-center pe-4'>
                <h4 className='m-0  text-danger'>$ {price}</h4>
                { purpose=='remove'?<div className='d-flex align-items-center gap-2 justify-content-center'><p className='fs-5 fw-bold m-0 text-secondary'>Quantity : {quantity} </p><i className="bi text-danger fs-3 bi-dash-circle-fill"  onClick={decrease}></i> <i onClick={increase}   className="bi text-success fs-3 bi-plus-circle-fill"></i> </div>:<></> }
